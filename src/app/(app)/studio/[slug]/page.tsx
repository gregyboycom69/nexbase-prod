...
function renderCtrl(c: Ctrl, isPreview: boolean = false, boundData: any = {}, rowSourceCache: Record<string, any[]> = {}) {
  ...
  if (c.type === 'TextBox') {
    ...
    if (isPreview) return <input type="text" placeholder={c.placeholder} value={displayValue} readOnly style={{...base,background:'#fff',border:'1px solid #e2e8f0',borderLeft:'3px solid #6366f1',padding:'0 12px',outline:'none'}} />
    return (
      <div style={{...base,background:'#fff',border:'1px solid #e2e8f0',borderLeft:'3px solid #6366f1',display:'flex',alignItems:'center',padding:'0 12px'}}>
        <span style={{color:'#9ca3af',fontSize:11}}>{c.placeholder || 'Type here...'}</span>
      </div>
    )
  }
  ...
  if (c.type === 'ComboBox') {
    ...
    return (
      <div style={{...base,background:'#fff',border:'1px solid #e2e8f0',display:'flex',alignItems:'center',padding:'0 12px',justifyContent:'space-between'}}>
        <span style={{color:'#9ca3af',fontSize:11}}>{c.placeholder || 'Select...'}</span>
  ...
  if (c.type === 'CheckBox') {
    ...
    return (
      <div style={{...base,background:'transparent',display:'flex',alignItems:'center',gap:8,padding:'0 12px'}}>
        ...
        <span style={{color:c.color}}>{c.caption}</span>
      </div>
    )
  }
    ...
    return (
      <div style={{...base,background:'#fff',border:'1px solid #e2e8f0',display:'flex',alignItems:'center',padding:'0 12px',justifyContent:'space-between'}}>
        <div style={{display:'flex',alignItems:'center',gap:8}}>
          ...
          <span style={{color:displayValue?c.color:'#9ca3af',fontSize:11}}>{displayValue || 'DD/MM/YYYY'}</span>
        </div>
  ...
  if (c.type === 'NumberBox') {
    ...
    return (
      <div style={{...base,background:'#fff',border:'1px solid #e2e8f0',display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 12px'}}>
        <span style={{color:c.color,fontSize:11,textAlign:'right',flex:1}}>{displayValue}</span>
  ...
  if (c.type === 'Lookup' || c.type === 'Search') {
    return (
      <div style={{...base,background:'#fff',border:'1px solid #e2e8f0',display:'flex',alignItems:'center',padding:'0 12px',justifyContent:'space-between'}}>
        <div style={{display:'flex',alignItems:'center',gap:8}}>
          <span style={{fontSize:14}}>🔍</span>
          <span style={{color:'#9ca3af',fontSize:11}}>Search or select...</span>
        </div>
        <div style={{background:'#6366f1',padding:'2px 8px',marginRight:-12,height:'100%',display:'flex',alignItems:'center',borderRadius:'0 4px 4px 0'}}>
    ...
    return (
      <div style={{...base,background:'#fff',border:'1px solid #e2e8f0',display:'flex',flexDirection:'column',overflow:'hidden'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(3, 1fr)',background:'#6366f1',color:'#fff',fontSize:10,fontWeight:600}}>
          {cols.map((col,i) => <div key={i} style={{padding:'4px 8px',borderRight:i<cols.length-1?'1px solid rgba(255,255,255,0.2)':'none'}}>{col}</div>)}
        </div>
        {[0,1,2].map(row => (
          <div key={row} style={{display:'grid',gridTemplateColumns:'repeat(3, 1fr)',background:row%2===0?'#fff':'#f8faff',fontSize:10,borderBottom:'1px solid #e2e8f0'}}>
            {cols.map((_,i) => <div key={i} style={{padding:'4px 8px',borderRight:i<cols.length-1?'1px solid #e2e8f0':'none',color:'#9ca3af'}}>---</div>)}
          </div>
        ))}
      </div>
    )
  ...
  if (c.type === 'Chart') {
    ...
    return (
      <div style={{...base,background:'#fff',border:'1px solid #e2e8f0',padding:16,display:'flex',flexDirection:'column'}}>
        <div style={{fontSize:12,fontWeight:600,color:'#1f2937',marginBottom:12}}>{c.caption || 'Chart'}</div>
      ...
      <div style={{...base,background:'rgba(0,0,0,0.1)',display:'flex',alignItems:'center',justifyContent:'center',padding:20}}>
        <div style={{background:'#fff',borderRadius:8,boxShadow:'0 10px 25px rgba(0,0,0,0.15)',width:'80%',maxHeight:'80%',display:'flex',flexDirection:'column',overflow:'hidden'}}>
          <div style={{background:'#6366f1',color:'#fff',padding:'8px 16px',display:'flex',justifyContent:'space-between',alignItems:'center',fontSize:12,fontWeight:600}}>
            <span>{c.caption || 'Modal Title'}</span>
  ...
  if (c.type === 'ProgressBar') {
    ...
    return (
      <div style={{...base,background:'transparent',display:'flex',alignItems:'center',gap:8,padding:'0 12px'}}>
        ...
        <span style={{fontSize:11,color:c.color,fontWeight:600,minWidth:35}}>{percent}%</span>
      </div>
    )
  }
  ...
  if (c.type === 'Badge') {
    return (
      <div style={{...base,background:'transparent',display:'flex',alignItems:'center',justifyContent:'center',padding:'0 12px'}}>
        <div style={{background:c.bg||'#6366f1',color:'#fff',padding:'4px 12px',borderRadius:12,display:'flex',alignItems:'center',gap:6,fontSize:11}}>
          <div style={{width:6,height:6,background:'#fff',borderRadius:'50%'}}/>
          <span>{c.caption || 'Badge'}</span>
        </div>
      </div>
    )
  }
  ...
  if (c.type === 'SectionHeader') {
    return (
      <div style={{...base,background:'#f9fafb',display:'flex',alignItems:'center',padding:'0 12px',gap:12}}>
        <span style={{fontSize:12,fontWeight:600,color:'#1f2937'}}>{c.caption || 'Section'}</span>
      ...
      <div style={{...base,background:'#fafbfc',border:'2px dashed #d1d5db',display:'flex',flexDirection:'column',overflow:'hidden',borderRadius:4}}>
        ...
        <div style={{flex:1,padding:8}}>
          <div style={{display:'grid',gridTemplateColumns:'repeat(3, 1fr)',gap:4}}>
            ...
            {[0,1,2].map(row => (
              <>
                <div key={`${row}-0`} style={{background:'#fff',padding:'4px 6px',fontSize:9,color:'#9ca3af',border:'1px solid #e5e7eb'}}>---</div>
                <div key={`${row}-1`} style={{background:'#fff',padding:'4px 6px',fontSize:9,color:'#9ca3af',border:'1px solid #e5e7eb'}}>---</div>
                <div key={`${row}-2`} style={{background:'#fff',padding:'4px 6px',fontSize:9,color:'#9ca3af',border:'1px solid #e5e7eb'}}>---</div>
              </>
            ))}
          </div>
        </div>
  ...
  // Fallback for any other control types
